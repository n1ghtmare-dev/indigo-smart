...
function renderHistoryPicker(groups){
  ...
  for(const group of groups){const checked=historySelectedKeys.has(group.key),selectedIndex=selected.findIndex(item=>item.key===group.key),item=el('article',undefined,`history-plan-choice${checked?' selected':''}`),choice=el('label',undefined,'history-plan-select'),input=el('input'),index=el('span',checked?String(selectedIndex+1):'+','history-plan-index'),copy=el('span',undefined,'history-plan-copy'),limitReached=!checked&&historySelectedKeys.size>=4;input.type='checkbox';input.checked=checked;input.disabled=limitReached;if(limitReached)input.dataset.readonly='true';input.setAttribute('aria-label',`Сравнивать план ${historyPlanName(group.name)}`);item.title=historyPlanName(group.name);if(checked){item.style.setProperty('--plan-color',historyPlanColors[selectedIndex]);index.style.setProperty('--plan-color',historyPlanColors[selectedIndex]);}copy.append(el('strong',historyPlanName(group.name)),el('small',fmtTime(group.latest?.calculated_at_utc)));choice.append(input,index,copy);input.onchange=()=>{if(input.checked){if(historySelectedKeys.size>=4){input.checked=false;return;}historySelectedKeys.add(group.key);}else historySelectedKeys.delete(group.key);renderHistory();};const tests=el('div',undefined,'history-test-buttons');tests.append(historyTestButton(group.base,'База'),historyTestButton(group.stress,'Стресс'));item.append(choice,tests);root.append(item);}
}
function renderHistoryInsight(groups){
  const root=$('history-insight'),candidates=groups.map(group=>({group,run:historyRun(group)})).filter(item=>item.run?.status==='FEASIBLE'&&historyValue(item.run,'pv_mln')!==null).sort((a,b)=>historyValue(a.run,'pv_mln')-historyValue(b.run,'pv_mln'));root.replaceChildren();
  if(!candidates.length){root.className='history-insight warning';root.append(el('strong','Нет сопоставимой PV'),el('span','В выбранном сценарии ни один рассчитанный план пока не покрывает условия.'));return;}
  const best=candidates[0],scenario=historyScenario==='BASE'?'базовом тесте':'стресс-тесте',open=el('button','Открыть','quiet compact');open.onclick=()=>openSavedRun(best.run);root.className='history-insight success';root.append(el('strong',`Минимальная PV в ${scenario}`),el('span',`${historyPlanName(best.group.name)}: ${fmt(best.run.pv_mln)} млн у.е.`),open);
}
...
function renderHistory(){
  const groups=historyGroups();$('history-count').textContent=groups.length;$('history-empty').hidden=!!groups.length;$('history-dashboard').hidden=!groups.length;if(!groups.length){$('comparison').replaceChildren();historySelectedKeys.clear();return;}chooseHistoryScenario(groups);chooseHistoryPlans(groups);document.querySelectorAll('[data-history-scenario]').forEach(button=>{const active=button.dataset.historyScenario===historyScenario;button.setAttribute('aria-pressed',String(active));button.onclick=()=>{historyScenario=button.dataset.historyScenario;renderHistory();};});renderHistorySummary(groups);renderHistoryInsight(groups);renderHistoryPicker(groups);renderHistoryCharts(groups);renderHistoryTable(groups);
}
