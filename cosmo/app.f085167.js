...
function refreshResultFreshness(){
  ...
  else{const [badge,badgeClass]=badgeStates[s.status]||['Проверьте результат','warning'];$('result-title').textContent=labels[s.status]||'Результат расчёта';$('result-badge').textContent=badge;$('result-badge').className=`badge ${badgeClass}`;if(summaryHeading)summaryHeading.textContent=(resultStates[s.status]||['Проверьте результат'])[0];summary.className=`result-summary ${s.status==='FEASIBLE'?'success':s.status==='INVALID_PLAN'?'danger':'warning'}`;}
...
function runStatus(run){
  ...
  if(run.status==='STOPPED_OVERFLOW')return {kind:'warning',symbol:'!',label:'Нужно пересчитать'};
  if(run.status==='INVALID_PLAN')return {kind:'error',symbol:'×',label:'Некорректные входы'};
...
const historyMetrics=[
  ...
  {key:'rejected_t',title:'Не принято складом',short:'Не принято',unit:'тонн',better:'Меньше лучше',lower:true,format:value=>`${fmt(value)} т`},
...
function chooseHistoryPlans(groups){
  const available=new Set(groups.map(group=>group.key));historySelectedKeys=new Set([...historySelectedKeys].filter(key=>available.has(key)).slice(0,4));if(historySelectionInitialized||historySelectedKeys.size){historySelectionInitialized=true;return;}
  ...
  for(const group of ranked.slice(0,4))historySelectedKeys.add(group.key);historySelectionInitialized=true;
}
function historyDecision(groups){
  ...
  const tieBreak=['shortage_t','rejected_t','worst_critical_service','pv_mln','capex_mln'];pool.sort((left,right)=>{if(Math.abs(right.score-left.score)>1e-9)return right.score-left.score;for(const key of tieBreak){const metric=historyMetrics.find(item=>item.key===key),a=historyValue(left.run,key),b=historyValue(right.run,key);if(a===null&&b!==null)return 1;if(a!==null&&b===null)return -1;if(a!==null&&b!==null&&Math.abs(a-b)>1e-9)return metric.lower?a-b:b-a;}return historyPlanName(left.group.name).localeCompare(historyPlanName(right.group.name),'ru');});
...
const resultStates={
  ...
  INFEASIBLE:['Расчёт выполнен полностью','Все показатели рассчитаны до конца горизонта. Ниже показаны ограничения текущего плана.'],
  TARGETS_MISSED:['Сервис текущего плана ниже целевого уровня',''],
  INVALID_PLAN:['Исправьте входные данные','Противоречие во входных данных не позволяет запустить расчёт.'],
  STOPPED_OVERFLOW:['Пересчитайте сохранённый план','Эта запись создана старой версией модели, которая останавливала расчёт при переполнении склада.']
};
const badgeStates={FEASIBLE:['Спрос покрыт','success'],INFEASIBLE:['Есть ограничения','warning'],TARGETS_MISSED:['Сервис ниже цели','warning'],INVALID_PLAN:['Исправьте входы','danger'],STOPPED_OVERFLOW:['Нужно пересчитать','warning']};
...
function violationFact(v,r){
  ...
  if(v.code==='STORAGE_OVERFLOW'){
    ...
    return diagnostic&&arrivals?`${humanDate(diagnostic.at)} партии ${arrivals} создали бы запас ${fmt(v.actual)} т при вместимости ${storage} ${fmt(v.limit)} т. Склад принял только доступную часть.`:`Поставка создала бы запас ${fmt(v.actual)} т при вместимости склада ${fmt(v.limit)} т. Склад принял только доступную часть.`;
  }
...
function groupViolations(rows,r){
  const groups=[],byKey=new Map();
  for(const violation of rows){
    const help=violationHelp(violation,r),key=`${violation.code}\u0000${help}`;let group=byKey.get(key);
    if(!group){group={code:violation.code,help,items:[]};byKey.set(key,group);groups.push(group);}group.items.push(violation);
  }
  return groups;
}
function violationMeasure(v){
  if(v.code==='OPENING_RESERVE_SHORTFALL')return [`${v.subject} год`,`Запас ${fmt(v.actual)} т`,`Норматив ${fmt(v.limit)} т`];
  if(v.code==='TOTAL_SERVICE_SHORTFALL'||v.code==='CRITICAL_SERVICE_SHORTFALL')return [`${v.subject} год`,`Обслужено ${fmt(Number(v.actual)*100)}%`,`Цель ${fmt(Number(v.limit)*100)}%`];
  if(v.code==='LOSS_CEILING_EXCEEDED')return [`${v.subject} год`,`Потери ${fmt(Number(v.actual)*100)}%`,`Предел ${fmt(Number(v.limit)*100)}%`];
  return [String(v.subject||'План'),`Факт ${fmt(v.actual)}`,`Предел ${fmt(v.limit)}`];
}
function appendViolationFacts(copy,group,r){
  if(group.items.length===1){copy.append(el('p',violationFact(group.items[0],r)));return;}
  if(group.code==='STORAGE_OVERFLOW'){const diagnostic=overflowDiagnostic(r);copy.append(el('p',`Ограничение вместимости сработало ${fmt(group.items.length,0)} раз. Всего склад не принял ${fmt(diagnostic?.rejected_t||0)} т.`));return;}
  const facts=el('div',undefined,'violation-facts');
  for(const violation of group.items){const row=el('div',undefined,'violation-fact');for(const value of violationMeasure(violation))row.append(el('span',value));facts.append(row);}copy.append(facts);
}
function appendViolationTechnical(details,group){
  details.append(el('summary','Технические детали'),el('code',group.code));const values=el('div',undefined,'violation-technical-values');
  for(const violation of group.items)values.append(el('span',`${violation.subject||'План'}: факт ${fmt(violation.actual)}, предел ${fmt(violation.limit)}.`));details.append(values);
}
function resultSummaryText(s,r,rows,defaultText){
  ...
  if(s.status==='INFEASIBLE'&&rows.some(row=>row.code==='STORAGE_OVERFLOW')){const rejected=Number(s.rejected_t||overflowDiagnostic(r)?.rejected_t||0);return `Расчёт выполнен до конца горизонта. Склад не смог принять ${fmt(rejected)} т поставок; этот объём исключён из доступного запаса и показан как ограничение плана.`;}
...
function appendOverflowDetails(summary,r,v){
  ...
  if(Number(diagnostic?.rejected_t)>0)copy.append(el('p',`За весь горизонт не принято ${fmt(diagnostic.rejected_t)} т в ${fmt(diagnostic.events_count,0)} событиях. Расчёт при этом продолжен до конца.`));
...
function renderInventoryChart(r){
  const card=$('inventory-chart-card'),root=$('inventory-chart'),timeline=r.inventory_timeline||{},points=(timeline.points||[]).map(point=>({...point,at:Number(point.at),inventory_t:Number(point.inventory_t),capacity_t:Number(point.capacity_t),arrival_t:Number(point.arrival_t),offered_t:Number(point.offered_t??point.arrival_t),rejected_t:Number(point.rejected_t||0),deliveries:(point.deliveries||[]).map(delivery=>({...delivery,gross_t:Number(delivery.gross_t)}))})).filter(point=>Number.isFinite(point.at)&&Number.isFinite(point.inventory_t)&&Number.isFinite(point.capacity_t));
  const arrivals=points.filter(point=>point.kind==='arrival'&&point.offered_t>0),limitedArrivals=arrivals.filter(point=>point.rejected_t>1e-9);
  ...
  const annual=(r.annual||[]).map(row=>({...row,year:Number(row.year),opening_t:Number(row.opening_t),shortage_t:Number(row.shortage_t)})),annualByYear=new Map(annual.map(row=>[row.year,row]));
  const reserveShortfalls=reserves.map(marker=>({...marker,actual_t:annualByYear.get(Number(marker.year))?.opening_t})).filter(marker=>Number.isFinite(marker.actual_t)&&marker.actual_t+1e-9<marker.required_reserve_t);
  const demandShortages=annual.filter(row=>Number.isFinite(row.shortage_t)&&row.shortage_t>1e-9);
  ...
  const legend=el('div',undefined,'chart-legend'),legendItems=[['Фактический запас','inventory'],['Вместимость склада','capacity'],['Норматив на начало года','reserve']];if(reserveShortfalls.length)legendItems.push(['Ниже норматива','shortfall']);if(demandShortages.length)legendItems.push(['Дефицит спроса','shortage']);legendItems.push(['Событие поставки','arrival']);if(limitedArrivals.length)legendItems.push(['Принято частично','rejected']);if(emergencyArrivals.length)legendItems.push(['Канал E','emergency']);for(const [label,kind]of legendItems){const item=el('span',undefined,'chart-legend-item'),mark=el('i',undefined,`chart-legend-mark ${kind}`);mark.setAttribute('aria-hidden','true');item.append(mark,document.createTextNode(label));legend.append(item);}
  ...
  const inventoryPath=points.map((point,index)=>`${index?'L':'M'}${x(point.at).toFixed(2)},${y(point.inventory_t).toFixed(2)}`).join(' '),inventoryArea=`${inventoryPath} L${x(points.at(-1).at).toFixed(2)},${y(0).toFixed(2)} L${x(points[0].at).toFixed(2)},${y(0).toFixed(2)} Z`;
  ...
  for(const marker of reserves){if(marker.at<minX||marker.at>maxX)continue;const shortfall=reserveShortfalls.find(item=>Number(item.year)===Number(marker.year)),dot=svgNode('circle',{cx:x(marker.at),cy:y(marker.required_reserve_t),r:4.5,class:`chart-reserve-point${shortfall?' shortfall':''}`});dot.append(svgNode('title',{},shortfall?`${marker.year}: запас ${fmt(shortfall.actual_t)} т, норматив ${fmt(marker.required_reserve_t)} т, ниже на ${fmt(marker.required_reserve_t-shortfall.actual_t)} т`:`${marker.year}: норматив ${fmt(marker.required_reserve_t)} т`));frame.append(dot);if(shortfall){const px=x(marker.at),actualY=y(shortfall.actual_t),requiredY=y(marker.required_reserve_t),gap=svgNode('line',{x1:px,y1:actualY,x2:px,y2:requiredY,class:'chart-reserve-gap'}),actual=svgNode('circle',{cx:px,cy:actualY,r:4,class:'chart-shortfall-point'});gap.append(svgNode('title',{},`${marker.year}: запас ниже норматива на ${fmt(marker.required_reserve_t-shortfall.actual_t)} т`));actual.append(svgNode('title',{},`${marker.year}: фактический запас ${fmt(shortfall.actual_t)} т`));frame.append(gap,actual);if(width>=680)frame.append(svgNode('text',{x:px+7,y:Math.max(pad.top+11,(actualY+requiredY)/2+3),class:'chart-shortfall-label'},`−${fmt(marker.required_reserve_t-shortfall.actual_t)} т`));}}
  for(const row of demandShortages){const start=(row.year-firstYear)*365,end=Math.min(start+365,maxX);if(end<=minX||start>=maxX)continue;const x1=x(Math.max(start,minX)),x2=x(end),line=svgNode('line',{x1,y1:y(0)-1,x2,y2:y(0)-1,class:'chart-shortage-year'});line.append(svgNode('title',{},`${row.year}: дефицит спроса ${fmt(row.shortage_t)} т`));frame.append(line);if(width>=680)frame.append(svgNode('text',{x:(x1+x2)/2,y:y(0)-9,class:'chart-shortage-label','text-anchor':'middle'},`Дефицит ${fmt(row.shortage_t)} т`));}
  for(const point of arrivals){const sources=[...new Set(point.deliveries.map(delivery=>delivery.source))],emergency=sources.includes('E'),limited=point.rejected_t>1e-9,dot=svgNode('circle',{cx:x(point.at),cy:y(point.inventory_t),r:limited?5:emergency?4:2.3,class:`chart-arrival-point${emergency?' emergency':''}${limited?' rejected':''}`}),sourceText=sources.length?` по каналам ${sources.join(', ')}`:'',volumeText=limited?`предложено ${fmt(point.offered_t)} т, принято ${fmt(point.arrival_t)} т, не принято ${fmt(point.rejected_t)} т`:`принято ${fmt(point.arrival_t)} т`;dot.append(svgNode('title',{},`${humanDate(point.at)}: поставка${sourceText}, ${volumeText}; запас после поступления ${fmt(point.inventory_t)} т`));frame.append(dot);}
  const limitedAt=timeline.capacity_limited_at===null||timeline.capacity_limited_at===undefined||timeline.capacity_limited_at===''?NaN:Number(timeline.capacity_limited_at);if(Number.isFinite(limitedAt)){const limited=limitedArrivals.find(point=>Math.abs(point.at-limitedAt)<1e-6);if(limited){const px=x(limited.at),py=y(limited.inventory_t),anchor=px>width-210?'end':'start',tx=px+(anchor==='end'?-10:10);frame.append(svgNode('text',{x:tx,y:Math.max(pad.top+12,py-12),class:'chart-stop-label','text-anchor':anchor},`Принято частично ${humanDate(limited.at)}`));}}
  const rejectedTotal=limitedArrivals.reduce((sum,point)=>sum+point.rejected_t,0),storageSummary=capacityChanges.map(change=>` ${humanDate(change.at)} введено ${change.storageId}, вместимость ${fmt(change.from)} → ${fmt(change.to)} тонн.`).join(''),reserveSummary=reserveShortfalls.length?` Ниже норматива в ${reserveShortfalls.map(item=>item.year).join(', ')} годах.`:'',shortageSummary=demandShortages.length?` Дефицит спроса: ${demandShortages.map(item=>`${item.year} год, ${fmt(item.shortage_t)} т`).join('; ')}.`:'';svg.append(frame);root.append(legend,svg);root.setAttribute('aria-label',`Фактическая динамика запаса от ${fmt(points[0].inventory_t)} до ${fmt(points.at(-1).inventory_t)} тонн. Событий поставки: ${arrivals.length}. Поставок по каналу E: ${emergencyArrivals.length}.${reserveSummary}${shortageSummary}${storageSummary}${rejectedTotal>1e-9?` Склад не принял ${fmt(rejectedTotal)} т; расчёт продолжен до конца.`:''}`);
}
...
function kpiSignal(key,s){
  ...
  if(key==='rejected_t')return Math.abs(value)<=epsilon?{state:'ideal',label:'Принято полностью'}:{state:'warning',label:'Принято частично'};
...
function showResult(r){
  ...
  let [summaryTitle,summaryText]=resultStates[s.status]||['Проверьте результат','Посмотрите подробности расчёта ниже.'];const overflow=rows.find(row=>row.code==='STORAGE_OVERFLOW');if(overflow&&s.status==='INFEASIBLE')summaryTitle='Склад принял не весь объём';
  ...
  configureResultActions(rows,r);if(overflow)appendOverflowDetails(summary,r,overflow);
  summary.className=`result-summary ${s.status==='FEASIBLE'?'success':s.status==='INVALID_PLAN'?'danger':'warning'}`;
  renderInventoryChart(r);
  const metrics=[['total_mln','Общие затраты','млн у.е.',fmt],['pv_mln','Приведённая стоимость','млн у.е. / PV',fmt],['capex_mln','Инвестиции','млн у.е. / CAPEX',fmt],['rejected_t','Не принято складом','тонн',fmt],['shortage_t','Общий дефицит','тонн',fmt],['critical_shortage_t','Критический дефицит','тонн',fmt],['worst_total_service','Худший общий сервис','доля спроса',pct],['worst_critical_service','Худший критический сервис','доля критического спроса',pct],['minimum_inventory_t','Минимальный запас','тонн',fmt],['final_inventory_t','Конечный запас','тонн',fmt],['minimum_reserve_margin_t','Минимум сверх норматива','тонн',fmt]];
  ...
  $('annual-card').hidden=!r.annual.length;if(r.annual.length){const hasRejected=r.annual.some(a=>Number(a.rejected_gross_t||0)>1e-9),headings=hasRejected?['Год','Начало, т','Норматив, т','Предложено, т','Принято, т','Не принято, т','Потери, т','Выдано, т','Остаток, т','Дефицит, т','Крит. дефицит, т','Общий сервис','Крит. сервис']:['Год','Начало, т','Норматив, т','Принято, т','Потери, т','Выдано, т','Остаток, т','Дефицит, т','Крит. дефицит, т','Общий сервис','Крит. сервис'];table($('annual'),headings,r.annual,(tr,a)=>{cell(tr,String(a.year));for(const key of ['opening_t','required_reserve_t'])cell(tr,fmt(a[key]));if(hasRejected){cell(tr,fmt(a.offered_gross_t));cell(tr,fmt(a.gross_t));cell(tr,fmt(a.rejected_gross_t));}else cell(tr,fmt(a.gross_t));for(const key of ['losses_t','served_t','closing_t','shortage_t','shortage_critical_t'])cell(tr,fmt(a[key]));cell(tr,pct(a.total_service));cell(tr,pct(a.critical_service));});}else $('annual').replaceChildren();
  ...
  renderAlternatives(r);const compactOverflow=s.status==='STOPPED_OVERFLOW'&&rows.length===1&&rows[0].code==='STORAGE_OVERFLOW',severity=overflow&&s.status==='INFEASIBLE'?'Поставки приняты частично':{INVALID_PLAN:'Некорректные входы',STOPPED_OVERFLOW:'Нужно пересчитать',INFEASIBLE:'Влияет на результат',TARGETS_MISSED:'Сервис ниже цели'}[s.status]||'Требует внимания';$('violations-card').hidden=!rows.length||compactOverflow;$('violations-title').textContent='Ограничения текущего плана';$('violations-summary').textContent=severity;$('violations-summary').className=`violation-severity severity-${s.status==='INVALID_PLAN'?'stopped':s.status==='TARGETS_MISSED'?'target':'failed'}`;$('violations').replaceChildren();
