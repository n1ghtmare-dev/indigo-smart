...
function renderInventoryChart(r){
  ...
  const arrivals=points.filter(point=>point.kind==='arrival'&&point.arrival_t>0);
  ...
  for(const point of arrivals){const sources=[...new Set(point.deliveries.map(delivery=>delivery.source))],emergency=sources.includes('E'),dot=svgNode('circle',{cx:x(point.at),cy:y(point.inventory_t),r:emergency?4:2.3,class:`chart-arrival-point${emergency?' emergency':''}`}),sourceText=sources.length?` по каналам ${sources.join(', ')}`:'';dot.append(svgNode('title',{},`${humanDate(point.at)}: поставка${sourceText}, ${fmt(point.arrival_t)} т; запас после поступления ${fmt(point.inventory_t)} т`));frame.append(dot);}
  ...
  const storageSummary=capacityChanges.map(change=>` ${humanDate(change.at)} введено ${change.storageId}, вместимость ${fmt(change.from)} → ${fmt(change.to)} тонн.`).join(''),reserveSummary=reserveShortfalls.length?` Ниже норматива в ${reserveShortfalls.map(item=>item.year).join(', ')} годах.`:'',shortageSummary=demandShortages.length?` Дефицит спроса: ${demandShortages.map(item=>`${item.year} год, ${fmt(item.shortage_t)} т`).join('; ')}.`:'';svg.append(frame);root.append(legend,svg);root.setAttribute('aria-label',`Фактическая динамика запаса от ${fmt(points[0].inventory_t)} до ${fmt(points.at(-1).inventory_t)} тонн. Событий поставки: ${arrivals.length}. Поставок по каналу E: ${emergencyArrivals.length}.${reserveSummary}${shortageSummary}${storageSummary}${Number.isFinite(stoppedAt)?` Расчёт остановлен ${humanDate(stoppedAt)} из-за переполнения.`:''}`);
}
