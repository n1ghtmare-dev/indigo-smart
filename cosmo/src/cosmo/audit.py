...
def audit_result(result):
    ...
    with localcontext() as ctx:
        ...
        for i, row in enumerate(rows):
            equal(f"mass:{i}", row["opening_t"] + row["gross_t"] - row["losses_t"] - row["served_t"], row["closing_t"])
            ...
            dt = row["until"] - row["at"]
            area = (row["opening_t"] + row["gross_t"] - row["losses_t"] + row["closing_t"]) * Decimal(dt.numerator) / Decimal(dt.denominator) / 730
        ...
        for year in result["physical"]["annual"]:
            ...
            for field in ("gross_t", "losses_t", "served_t", "served_critical_t", "shortage_t", "holding_mln"):
                equal(f"year:{year['year']}:{field}", sum((r[field] for r in subset), Decimal(0)), year[field])
            equal(f"year_balance:{year['year']}", year["opening_t"] + year["gross_t"] - year["losses_t"] - year["served_t"], year["closing_t"])
